...
<script lang="ts">
	...
	let filteredPlayers = $derived(players.filter((p: any) => p.gender === activeGender));
	...
	$effect(() => {
		selected = filteredPlayers[0] ?? null;
		flipped = false;
	});
...
>
	...
	<div class="col-span-8 grid grid-cols-1 gap-8 md:grid-cols-[360px_1fr]">
		<div
			...
			{#if selected}
				<div class="flip-card h-full w-full" class:flipped>
					<div class="flip-card-inner h-full w-full rounded-[10px]">
						...
						<div class="flip-card-face flip-card-back bg-dark">
							{#if selected.highlight_video}
								...
								<button
									type="button"
									onclick={openModal}
									class="text-button absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded bg-white px-4 py-2 text-dark uppercase opacity-0 transition-opacity duration-200 group-hover:opacity-100"
								>
									Play video
								</button>
							{/if}
		...
		<div class="flex flex-col">
			<div
				class="hidden grid-cols-[1.6fr_1fr_1fr_1.2fr_auto] items-center gap-4 pb-3 text-xs tracking-wide text-dark/40 uppercase sm:grid"
			>
				<span>Nom</span>
				<span>Position</span>
				<span>Pays</span>
			...
			{#each filteredPlayers as player}
				<button
					type="button"
					class="grid grid-cols-1 items-center gap-2 border-b border-dotted border-dark/20 py-4 text-left transition-colors hover:bg-dark/3 sm:grid-cols-[1.6fr_1fr_1fr_1.2fr_auto] sm:gap-4"
					onmouseenter={() => handleRowEnter(player)}
					onfocus={() => handleRowEnter(player)}
				>
					<span class="text-body-sm font-jakarta font-semibold text-primary">{player.name}</span>
					<span class="text-body-sm font-jakarta text-dark">{player.position}</span>
					...
					<span class="text-body-sm font-jakarta text-dark">{player.club}</span>
